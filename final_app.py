...
def recommend_recipes(
    ...
    if sort_by == "Macro score":
        data = data.sort_values(["macro_score", "final_score", "ingredient_score"], ascending=[False, False, False])
    elif sort_by == "Ingredient score":
        data = data.sort_values(["ingredient_score", "final_score", "macro_score"], ascending=[False, False, False])
    elif sort_by == "Calories":
        data = data.sort_values("calories", ascending=True)
    else:
        data = data.sort_values(["nutrient_density_norm", "final_score", "macro_score"], ascending=[False, False, False])
...
def render_why_recipe_streamlit(row: pd.Series, per_meal_calories: float,
    ...
    serving_g = pd.to_numeric(row.get("serving_g"), errors="coerce")
    serving_text = f"{int(round(serving_g))} g" if pd.notna(serving_g) and serving_g > 0 else "N/A"
    remaining_daily_cal = safe_float(row.get("remaining_daily_calories"), 0.0)
    ...
    with r7:
        st.caption("Sugar")
        st.markdown(f"**{float(sugar_val):.1f} g**" if pd.notna(sugar_val) else "**N/A**")
    with r8:
        st.caption("Sodium")
        st.markdown(f"**{float(sodium_val):.0f} mg**" if pd.notna(sodium_val) else "**N/A**")
    with r9:
